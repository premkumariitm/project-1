...
type ExamType = 'MAINS' | 'ADVANCE';
type Difficulty = 'EASY' | 'MEDIUM' | 'HARD';

interface SubjectMatrix {
    mains: { easy: number; medium: number; hard: number };
    advance: { easy: number; medium: number; hard: number };
}
...
const SPECIAL_BADGES_DATA: Record<number, { icon: string, name: string }> = {
    50: { icon: '🌌', name: 'Nebula Voyager' },
    100: { icon: '☄️', name: 'Comet Rider' },
    150: { icon: '🪐', name: 'Ring Master' },
    200: { icon: '🌟', name: 'Nova King' },
    250: { icon: '🌀', name: 'Galaxy Architect' },
    300: { icon: '🧿', name: 'Cosmic Entity' },
    350: { icon: '💎', name: 'Eternal Legend' }
};
...
// Fixed: Added Record<string, number> type to history prop
const SolveHistoryGraph = ({ history = {} as Record<string, number>, label = "Your progress" }) => {
    ...
    return (
        <div className="bg-[#06241c] border border-emerald-900/50 rounded-3xl p-6 shadow-2xl w-full relative group transition-all duration-300">
            ...
            {celebration.type === 'LEVEL_UP' && (
                <div className="text-center space-y-8 pointer-events-none">
                    <div className="relative">
                        ...
                        <div className="absolute -bottom-10 -left-10 text-6xl animate-bounce delay-150">🎉</div>
                    </div>
            ...
            {celebration.type === 'STREAK' && (
                <div className="fixed inset-0 flex items-center justify-center pointer-events-auto" onClick={onDismiss}>
                    ...
                    <div className="relative z-10 text-center space-y-4 max-w-lg w-full px-6 py-20 rounded-[4rem] burning-page overflow-hidden">
                        ...
                        <div className="relative animate-flicker-intense text-8xl md:text-9xl mb-4">🔥</div>
...
const StreakCard = ({ streak = 0, label = "Streak" }) => (
    <div className={`relative bg-[#06241c] border ${streak > 0 ? 'border-orange-500 shadow-[0_0_20px_rgba(249,115,22,0.15)]' : 'border-orange-500/20'} rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center text-center group hover:border-orange-500/60 transition-all min-h-[110px] overflow-hidden`}>
        <span className={`text-2xl md:text-4xl mb-1 flex-shrink-0 ${streak > 0 ? 'drop-shadow-[0_0_8px_#f97316]' : ''}`}>🔥</span>
...
const PointsCard = ({ points, label = "Points" }) => (
    <div className="bg-[#06241c] border border-yellow-500/20 rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center text-center group hover:border-yellow-500/50 transition-all shadow-lg min-h-[110px]">
        <span className="text-2xl md:text-4xl mb-1 flex-shrink-0">⚡</span>
...
const ActionModal = ({ isOpen, onClose, subject, examType, difficulty, initialPoints, onSave }) => {
    ...
    const inc = (POINT_SCHEME as any)[subject]?.[examType]?.[difficulty] || 10;
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 modal-overlay fade-in" onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div className="bg-[#06241c] border border-emerald-500/40 rounded-3xl p-6 md:p-10 w-full max-w-lg space-y-6 shadow-2xl overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between pb-4 border-b border-emerald-900/30">
                    <button onClick={onClose} className="bg-red-950/20 hover:bg-red-900/30 text-red-400 border border-red-900/40 w-10 h-10 md:w-14 md:h-14 flex items-center justify-center rounded-xl font-game text-2xl md:text-3xl transition-all active:scale-95 leading-none">×</button>
                ...
                <div className="text-center space-y-1">
                    ...
                    <p className="text-[10px] md:text-xs text-emerald-600 font-bold tracking-widest uppercase">{examType} • {difficulty} (+{inc})</p>
                </div>
                <div className="bg-emerald-950/80 border-2 border-emerald-500/30 rounded-2xl py-8 md:py-12 text-center relative overflow-hidden shadow-inner">
                    <div className="text-yellow-500 font-game text-4xl md:text-7xl font-bold tracking-tighter flex items-center justify-center gap-3 overflow-hidden px-4">
                        <span className="text-3xl md:text-5xl shrink-0">⚡</span>{localPoints}
                    </div>
...
const DashboardView = ({ globalStats, globalHistory, onEnterWorld }) => {
    const levelBadges = useMemo(() => {
        ...
        for (let i = 10; i <= globalStats.level; i += 10) {
            ...
            list.push({ icon: '🏆', text: `${stageName} stage Keep going`, key: `lvl-${i}` });
        }
        return list;
    }, [globalStats.level]);
    ...
    const streakBadges = useMemo(() => {
        ...
        for (let i = 10; i <= globalStats.streak; i += 10) {
            ...
            list.push({ icon: '🔥🔥🔥', text: `Streaker Go ${streakCount}`, key: `streak-${i}` });
        }
        return list;
    }, [globalStats.streak]);
        ...
        <div className="fade-in space-y-8 md:space-y-12 pb-20">
            ...
            <section className="bg-emerald-950/20 border border-emerald-800/40 rounded-[2rem] p-6 md:p-10 space-y-10 md:space-y-14 relative overflow-hidden shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                    {(['Physics', 'Chemistry', 'Mathematics'] as Subject[]).map((subject, idx) => (
                        <button key={subject} onClick={() => onEnterWorld(subject)} className="btn-3d-emerald rounded-2xl md:rounded-3xl p-6 md:p-8 flex flex-col items-center justify-center gap-4 group transition-all hover:scale-[1.02]">
                            <div className="w-14 h-14 md:w-20 md:h-20 rounded-full bg-emerald-950/50 flex items-center justify-center border border-emerald-500/20 group-hover:border-emerald-500 transition-all duration-300 shadow-lg aspect-square overflow-hidden shrink-0">
                                <span className="text-3xl md:text-5xl">{idx === 0 ? '⚛️' : idx === 1 ? '🧪' : '🧮'}</span>
                            </div>
            ...
            <section className="bg-[#06241c] border border-emerald-900/30 rounded-3xl p-6 md:p-10 space-y-8 md:space-y-12 shadow-inner">
                <div className="flex justify-center">
                    <div className="bg-[#051a14] border border-emerald-500/20 rounded-xl px-10 md:px-14 py-3">
                        <h2 className="text-emerald-500 font-game text-xs md:text-xl font-bold uppercase tracking-[0.4em]">Trophy Room</h2>
                    </div>
                </div>
...
const WorldView = ({ subject, onBack, subjectData, onUpdatePoint }) => {
    ...
    const getIcon = () => subject === 'Physics' ? '⚛️' : (subject === 'Chemistry' ? '🧪' : '🧮');
    ...
    return (
        <div className="fade-in space-y-8 md:space-y-10 pb-20">
            <header className="bg-emerald-950/20 border border-emerald-900/50 rounded-2xl p-4 md:p-6 flex items-center justify-between gap-4 sticky top-4 z-40 backdrop-blur-md shadow-xl">
                <button onClick={onBack} className="bg-[#06241c] border border-emerald-500/30 px-4 md:px-6 py-2 rounded-xl font-game text-[8px] md:text-xs text-emerald-400 uppercase tracking-widest hover:bg-emerald-900/40 transition-all active:scale-95 leading-none">← Back</button>
...
const App = () => {
    ...
    const [activeWorld, setActiveWorld] = useState<Subject | null>(null);
    const [celebration, setCelebration] = useState<any>(null);

    // Fixed: Added AppData type to state initialization
    const [data, setData] = useState<AppData>(() => {
        const saved = localStorage.getItem('jee-universe-final-v13');
    ...
    const stats = useMemo(() => {
        const subStats: Record<string, any> = {}; let gPoints = 0;
        (['Physics', 'Chemistry', 'Mathematics'] as Subject[]).forEach(sub => {
    ...
    useEffect(() => {
        ...
        if (lvlBadgeVal >= 10 && !milestones.includes(`lvl-${lvlBadgeVal}`)) {
            ...
            setCelebration({ type: 'BADGE', icon: '🏆', text: `${name} stage Keep going` });
        ...
        if (strBadgeVal >= 10 && !milestones.includes(`str-${strBadgeVal}`)) {
            setCelebration({ type: 'BADGE', icon: '🔥🔥🔥', text: `Streaker Go ${strBadgeVal/10}` });
    ...
    useEffect(() => localStorage.setItem('jee-universe-final-v13', JSON.stringify(data)), [data]);

    const updatePoints = (subject: Subject, type: string, difficulty: string, newValue: number) => {
        const today = new Date().toLocaleDateString('en-CA');
        setData(prev => {
            // Fixed: Type 'unknown' cannot be used as an index type
            const oldVal = (prev[subject].matrix as any)[type.toLowerCase()][difficulty.toLowerCase()];
            ...
            return { 
                ...
                [subject]: { 
                    ...
                    matrix: { 
                        ...prev[subject].matrix, 
                        [type.toLowerCase() as keyof SubjectMatrix]: { 
                            ...(prev[subject].matrix as any)[type.toLowerCase()], 
    ...
    return (
        <div className="min-h-screen bg-[#051a14] p-4 md:p-8 space-y-8 select-none overflow-x-hidden">
            ...
            <div className="container-main space-y-8 md:space-y-12">
                {view === 'dashboard' ? (
                    <DashboardView globalStats={stats.global} globalHistory={globalHistory} onEnterWorld={(w) => { setActiveWorld(w as Subject); setView('world'); window.scrollTo(0, 0); }} />
                ) : (
                    activeWorld && <WorldView subject={activeWorld} onBack={() => setView('dashboard')} subjectData={stats.subjects[activeWorld]} onUpdatePoint={updatePoints} />
                )}
            </div>
        </div>
    );
